...
1. Init Project
```shell
kubebuilder init --domain shawnrong.github.io --repo shawnrong.github.io/examplecontroller
```
2. Create API
```shell
...
```sh
make docker-build docker-push IMG=<some-registry>/sample-2:tag
```
	
3. Deploy the controller to the cluster with the image specified by `IMG`:

```sh
make deploy IMG=<some-registry>/sample-2:tag
```
